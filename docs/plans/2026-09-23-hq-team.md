...
- The runtime pane runs the dev server through `tee .hq/runtime.log` (overwritten per restart), so the runtime check greps a file instead of scraping scrollback; `.hq/.gitignore` excludes it.
...
```
and the usage printer to `sed -n '2,11p'`.
...
for s in "Gate 1" "Gate 2" "hq respawn" "herdr agent prompt" "herdr notification show" \
         ...
         "Mode: resume" "gh pr create" "hq/" "git status --porcelain" "autonomy=trusted"; do
...
2. `herdr pane run RT "<dev> 2>&1 | tee .hq/runtime.log"`
...
5. `grep -En "<errors>" .hq/runtime.log` → each match is a runtime finding.
...
Start a second run on a fresh fixture with brief "Add GET /version returning package.json version, with a test". During task 01, once `L-impl` is working, run `herdr pane send-keys <runtime pane> ctrl+c` and then `herdr pane run <runtime pane> "node -e 'throw new Error(\"boom\")' 2>&1 | tee .hq/runtime.log"`.
