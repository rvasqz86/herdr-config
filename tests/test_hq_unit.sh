...
# per-machine hq.local sits between the project's .hq/runtime and the defaults
printf 'kind_manager=omp  # no claude here\nkind_impl=copilot\n' >"$k/local"
assert_eq "$(HQ_LOCAL="$k/local" kind_for "$k" manager)" omp "kind_for reads hq.local"
assert_eq "$(HQ_LOCAL="$k/local" kind_for "$k" impl)" claude ".hq/runtime beats hq.local"
assert_eq "$(HQ_LOCAL="$k/missing" kind_for "$k" manager)" claude "missing hq.local is fine"
assert_eq "$(HQ_LOCAL="$k/local" code_tools)" "claude omp copilot cursor" "code_tools default"
echo 'code_tools=omp  copilot cursor' >"$k/local"
assert_eq "$(HQ_LOCAL="$k/local" code_tools)" "omp copilot cursor" "code_tools from hq.local"
echo 'code_tools=omp nope' >"$k/local"
HQ_LOCAL="$k/local" code_tools >/dev/null 2>&1 && bad "code_tools accepted unknown tool" || ok "code_tools rejects unknown tool"
echo 'code_tools=omp omp omp omp omp' >"$k/local"
HQ_LOCAL="$k/local" code_tools >/dev/null 2>&1 && bad "code_tools accepted 5 tools" || ok "code_tools allows at most 4"
echo 'code_tools=' >"$k/local"
assert_eq "$(HQ_LOCAL="$k/local" code_tools)" "claude omp copilot cursor" "empty code_tools means default"
